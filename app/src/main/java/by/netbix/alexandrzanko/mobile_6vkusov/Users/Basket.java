...
public class Basket implements LoadJson {
                ...
                if (sessionName.equals("add")){
                    try {
                        ...
                        if (!empty){
                            ...
                            String url = Singleton.currentState().getStore().getContext().getResources().getString(by.netbix.alexandrzanko.mobile_6vkusov.R.string.api_base);
    ...
    public void initBasketFromRegisterUser(){
        if (Singleton.currentState().getUser().getStatus() == STATUS.REGISTER){
            String url = Singleton.currentState().getStore().getContext().getResources().getString(by.netbix.alexandrzanko.mobile_6vkusov.R.string.api_basket_items);
    ...
    private void addProduct(Product product){
        ...
        boolean points = product.get_points() > 0 ? true : false;
        if (points){
            if (isFreeFoodExist) {
                return;
            }else{
        ...
        String url = Singleton.currentState().getStore().getContext().getResources().getString(by.netbix.alexandrzanko.mobile_6vkusov.R.string.api_add_variants);
        JSONObject params = new JSONObject();
        try {
    ...
    public void addProductItemOneRegister(ProductItem item){
        if(Singleton.currentState().getUser().getStatus() == STATUS.REGISTER) {
            boolean points = item.get_points() > 0 ? true : false;
    ...
    public void removeProductItem(ProductItem item){
        if(Singleton.currentState().getUser().getStatus() == STATUS.REGISTER){
            ...
            String url = Singleton.currentState().getStore().getContext().getResources().getString(by.netbix.alexandrzanko.mobile_6vkusov.R.string.api_remove_item);
            JSONObject params = new JSONObject();
            try {
                JSONObject userProfileJson = Singleton.currentState().getUser().getProfile();
                String session = userProfileJson.getString("session");
    ...
    public void minusProductItem(ProductItem item){
        if(Singleton.currentState().getUser().getStatus() == STATUS.REGISTER){
            ...
            String url = Singleton.currentState().getStore().getContext().getResources().getString(by.netbix.alexandrzanko.mobile_6vkusov.R.string.api_remove_variant);
            JSONObject params = new JSONObject();
            try {
                JSONObject userProfileJson = Singleton.currentState().getUser().getProfile();
                String session = userProfileJson.getString("session");
